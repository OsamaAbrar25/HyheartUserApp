...
export const userApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    createCall: builder.mutation({ query: (body) => ({ url: `call`, method: "POST", body }), invalidatesTags: ['Call'] }),
    updateCall: builder.mutation({ query: (body) => ({ url: `call/${body.id}`, method: "PATCH", body }), invalidatesTags: ['Call'] }),
    getCallHistory: builder.query<undefined, string>({ query: () => `call`, providesTags: ['Call'], }),
    getCreditHistory: builder.query<undefined, string>({ query: () => `credit/history`, providesTags: ['Payment', 'Call'], }),
    getTotalCredit: builder.query<undefined, string>({ query: () => `credit`, providesTags: ['Payment', 'Call'], }),
